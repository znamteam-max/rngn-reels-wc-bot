...
Футбольная страница использует компактный пользовательский `/assets/football-ticker-bg.png` как единое прозрачное изображение `1920x1080`; видимая плашка занимает нижние `54px`, responsive масштабирует весь stage целиком. Текстовая маска: `left: 0`, `right: 0`, `bottom: 3px`, `height: 48px`; font-size: `29px`. Зона `0–170px` полностью защищает логотип, в зоне `170–255px` текст плавно проявляется. Анимация начинается на `2000px` (`1920 + 80`) за правым краем сцены и заканчивается после `-(trackWidth + 320px)`. Диагностический overlay доступен через `?debug=1`; preview без параметров остаётся на чёрном фоне.
