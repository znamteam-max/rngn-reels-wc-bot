...
def _status() -> dict[str, Any]:
    ...
    project_rows = db.fetch_all("""
        SELECT COALESCE(NULLIF(project_code,''),'unassigned') project_code, count(*) count
        FROM videos WHERE status <> 'deleted' GROUP BY 1 ORDER BY 1
    """)
    return {
        "ok": True,
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                ON CONFLICT (code) DO UPDATE SET name=EXCLUDED.name, emoji=EXCLUDED.emoji,
                    is_active=false, sort_order=EXCLUDED.sort_order, updated_at=now()
                RETURNING id
            """, (ARCHIVE_CODE, ARCHIVE_NAME))
            archive_id = int(cur.fetchone()["id"])
            cur.execute("""
            ...
            if ids:
                cur.execute("""
                    UPDATE videos SET project_id=%s, project_code=%s, project_name=%s,
                        sheet_sync_status='queued', sheet_sync_error=NULL, updated_at=now()
                    WHERE id = ANY(%s)
                """, (archive_id, ARCHIVE_CODE, ARCHIVE_NAME, ids))
            db.log_event(conn, entity_type="sheet_reconciliation", entity_id=None,
                action="world_cup_2026_archived", actor_username="system:world-cup-2026-archive",
                after_data={"changed_count": len(ids), "changed_ids": ids})
    settings = get_settings()
    run_id = reconciliation.create_audit_run(actor_tg_id=0,
        actor_username="system:world-cup-2026-archive", chat_id=int(settings.admin_chat_id))
    return {"ok": True, "changed_count": len(ids), "changed_ids": ids, "run_id": run_id, "status": _status()}
...
def _confirm() -> dict[str, Any]:
    ...
    if run.get("status") == "awaiting_confirmation":
        changed = reconciliation.confirm_run(int(run["id"]), mode="db_only", actor_tg_id=0,
            actor_username="system:world-cup-2026-archive")
    return {"ok": True, "confirmed_now": changed, "status": _status()}
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            if action == "status": payload = _status()
            elif action == "apply": payload = _apply()
            elif action == "confirm": payload = _confirm()
            elif action == "drain": payload = {"ok": True, "worker": process_jobs(source="github_actions"), "status": _status()}
            else:
                self._send(400, {"ok": False, "error": "unsupported action"}); return
        except Exception as exc:
            self._send(500, {"ok": False, "error": f"{type(exc).__name__}: {exc}"[:500]}); return
