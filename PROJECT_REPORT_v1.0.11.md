...
Встроенный браузер Codex аварийно завершал приложение при открытии Vercel Dashboard. После четырёх повторов браузерный путь прекращён. Дальнейший деплой выполняется через GitHub integration и проверяется публичными endpoints и Telegram API.
