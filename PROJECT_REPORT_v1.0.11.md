...
Первый live-повтор ручного `12.07` выявил ошибку audit log: PostgreSQL возвращал `publish_date` как объект `date`, который передавался в JSONB без преобразования. Audit payload теперь использует ISO-строку. Дополнительно Telegram-ответ `message is not modified` сделан идемпотентным, поэтому повторная установка той же даты также не откатывает DB-транзакцию.

Для дальнейшей production-диагностики защищённый webhook-ответ дополнен безопасным типом и текстом исключения. Значение доступно только запросу с корректным `WEBHOOK_SECRET`; BOT_TOKEN маскируется, публичный health не раскрывает detail.
