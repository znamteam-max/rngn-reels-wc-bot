...
# 1) Add a report-only archived project. It is NOT added to PROJECTS, so it does
# not appear in the new-submission project picker.
replace_once(
    "bot/projects.py",
...
# 2) Reconciliation/statistics must know about archived report projects.
replace_once(
    "bot/reconciliation.py",
...
# 3) Legacy report helper in sheets should also resolve archived project names.
replace_once(
    "bot/sheets.py",
...
# 4) Temporary OIDC allowance for this one push-triggered ops workflow only.
replace_once(
    "bot/github_oidc.py",
...
# 5) Temporary protected endpoint used only for this migration.
endpoint = Path("api/internal/archive-world-cup-2026-5d9f7a2c91.py")
endpoint.write_text(r'''from __future__ import annotations
